...
import type { NewsSourceType } from '../../../types/hospital-news.types';
...
export const GetHospitalNewsParametersSchema = z.object({
  ...
  incremental: z.boolean().optional().default(false)
    .describe('是否只返回新增内容（基于上次查询时间）/ Only return new items since last query'),
}).strict();
...
export const GetHospitalNewsToolDefinition = {
...
- 自动分类（科研/临床/管理/政策等）
- 2小时缓存，平衡实时性与性能
- 百度搜索和微信搜索支持关键词叠加，提供更精准搜索

常用组合：可与 get_subscriptions 顺序使用，先获取订阅医院列表，再查询新闻`,
  ...
  metadata: {
    ...
    triggers: {
      keywords: [
        '医院新闻', '最新消息', '医院动态', 'news', 'hospital news',
        '更新', '最近', '新消息', '近况', '动态', '资讯',
        '查看', '查询', '搜索', '有什么', '最近有什么',
      ],
      patterns: [
        '.*医院.*新闻', '.*医院.*消息', '.*医院.*动态', '.*医院.*更新',
        '.*医院.*近况', '.*医院.*资讯', '.*医院.*什么',
        '查看.*医院.*', '查询.*医院.*', '最近.*医院.*',
        '.*订阅.*更新', '.*关注.*动态',
      ],
      ...
      before: [
        {
          ...
          reason: '获取用户订阅的医院列表，用于批量查询',
          autoSuggest: true,
        },
      ],
      after: [],
      parallelWith: [], // 可与多个医院新闻并行
    },
    resultUsage: {
      fields: {
        'data.results': '新闻列表',
        'data.results[*].title': '新闻标题',
        'data.results[*].sentiment': '情感倾向',
        'data.results[*].source.type': '消息来源类型',
      },
    },
  },
};

export function createGetHospitalNewsHandler(newsService: HospitalNewsService): ToolDefinition['handler'] {
